        ...
        <div className="console-panel h-full flex flex-col">
            ...
            <div className="space-y-4 flex-1">
                <div className="flex flex-wrap items-center gap-3 mb-1">
                    ...
                    {account.custom_label && (
                        <span className="flex items-center gap-1 px-2 py-0.5 rounded-md bg-gray-100 dark:bg-white/10 text-gray-600 dark:text-gray-300 text-[10px] font-medium shrink-0">
                ...
                {geminiProModel && (
                    <div className="space-y-1.5">
                        <div className="flex flex-wrap gap-1 justify-between items-baseline">
                        ...
                        <div className="w-full bg-gray-100 dark:bg-base-300 rounded-full h-1.5 overflow-hidden">
                            <div
                                className={`h-full rounded-full transition-all duration-700 ${geminiProModel.percentage >= 50 ? 'bg-blue-500' :
                ...
                {geminiImageModel && (
                    <div className="space-y-1.5">
                        ...
                        <div className="w-full bg-gray-100 dark:bg-base-300 rounded-full h-1.5 overflow-hidden">
                            <div
                                className={`h-full rounded-full transition-all duration-700 ${geminiImageModel.percentage >= 50 ? 'bg-blue-500' :
                ...
                {geminiFlashModel && (
                    <div className="space-y-1.5">
                        ...
                        <div className="w-full bg-gray-100 dark:bg-base-300 rounded-full h-1.5 overflow-hidden">
                            <div
                                className={`h-full rounded-full transition-all duration-700 ${geminiFlashModel.percentage >= 50 ? 'bg-blue-500' :
                    ...
                    <div className="space-y-1.5">
                        <div className="flex flex-wrap gap-1 justify-between items-baseline">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <span className={`text-xs font-bold ${claudeModel.percentage >= 50 ? 'text-emerald-600 dark:text-emerald-400' :
                        ...
                        <div className="w-full bg-gray-100 dark:bg-base-300 rounded-full h-1.5 overflow-hidden">
                            <div
                                className={`h-full rounded-full transition-all duration-700 ${claudeModel.percentage >= 50 ? 'bg-blue-500' :
                ...
                {!geminiProModel && !geminiFlashModel && !geminiImageModel && !claudeModel && (
                    <p className="py-4 text-sm console-muted">{t('console.overview_current_quota_missing', { defaultValue: i18n.language.startsWith('zh') ? '尚无模型配额数据，请刷新当前账号配额。' : 'No model quota data yet. Refresh the current account quota.' })}</p>
                )}
            </div>
            ...
            {onSwitch && (
                <div className="mt-auto pt-3">
                    <button
                        className="console-button w-full justify-center"
                        onClick={onSwitch}
                    >
