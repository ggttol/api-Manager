...
impl Account {
    ...
    pub fn update_quota(&mut self, mut quota: QuotaData) {
        // A failed/partial summary is not evidence that a previously observed
        // bucket recovered. Merge old buckets into the new response and only
        // replace a same-id bucket with a newer observation.
        if let Some(existing) = &self.quota {
            if let Some(old_groups) = &existing.quota_groups {
                ...
                for old_group in old_groups {
                    let index = groups
                        .iter()
                        .position(|group| group.display_name == old_group.display_name)
                        .unwrap_or_else(|| {
                    ...
                    for old_bucket in &old_group.buckets {
                        ...
                        if let Some(current) = groups[index]
                            .buckets
                            .iter_mut()
                            .find(|bucket| bucket.bucket_id == previous.bucket_id)
                        {
            ...
            if quota.subscription_tier.is_none() {
                quota.subscription_tier = existing
                    .subscription_tier
                    .clone()
                    .map(|tier| crate::models::quota::resolve_subscription_tier(Some(&tier)));
            }
        }
