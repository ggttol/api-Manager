...
两遍功能审计修复：请求日志的账号选项合并 Google、Codex 与历史日志账号，不再受当前页限制；账号、文本和错误筛选统一在数据库执行，列表与总数保持一致，支持 Codex 无邮箱标识及 HTTP 200 内的语义错误。修复日志详情、统计时间范围和配置加载的过期响应覆盖，以及并发配置保存与失败回滚问题。

修复账号和 OAuth 刷新凭据持久化、取消与回调竞态、Codex 会话及私有状态边界、代理池与配额调度、Responses 输入转换、工具和 SSE 终止语义、thinking 预算和签名恢复、令牌策略更新、可信代理 IP 与 CIDR、CLI 配置备份恢复等已确认缺陷。管理鉴权始终独立于推理鉴权开关；登录验证不再将服务端错误视为成功。凭据文件采用私有权限的原子写入，启动日志及多语言登录帮助不再披露或引导查找日志中的明文凭据。

启停控制串行化并显式报告配置持久化失败：启动未完成前置加载与保存时不开放推理，停止时立即关闭推理，即使保存失败也不重新开放。验证使用隔离数据目录、合成账号与本地 HTTP 场景；真实上游授权、生产推理及 Windows 运行时不属于本轮已执行验证。

缺失 Google 账号目录按空池处理，损坏或不可读取的存储仍返回错误；首次部署和仅 Codex 部署可正常重启共享网关，并有空目录与损坏存储边界回归。

修复后端未配置启动参数时返回 `null` 导致系统设置整页崩溃的问题；前端类型与可空契约一致，已保存参数和检测结果共用无损格式化逻辑。
...
修复混合搜索/客户端工具续聊绕过完整历史校验、搜索缓存写入失败遗留工具状态的问题；两项回归均验证修复前失败、修复后通过。32 项 Codex 与 5 项中间件回归通过。生产环境使用官方 Anthropic SDK 0.125.0 验证普通/流式搜索、URL 引文、两种返回的完整历史续接、篡改拒绝、严格 Schema 及搜索与 Schema 组合；Claude Code 2.1.263 实际执行 WebSearch 并完成回复，结构化输出也通过。桌面/390px、深色/浅色、中英文及示例复制已验证，账号首选和启用状态未改变。

新增 Codex 额度耗尽自动切换，Responses 与 Anthropic Messages 共用首选优先、跳过冷却账号的调度；完整 HTTP `429` 可切换可迁移请求，不修改首选或启停状态。冷却时间持久化并支持真实额度恢复，全部冷却返回 `429` / `Retry-After`。账号池显示冷却原因、恢复时间，并在额度查询批次完成后同步状态；中英文接入指南说明切换与会话边界。

旧文字会话可沿用原始缓存键和会话标识完成切换；私有状态独立记录来源，旧账号的加密上下文和工具状态不会误投新账号，新账号生成的状态仍可续接。修复无重置时间时误用未耗尽周窗口、旧设备授权结果清除新额度失败，以及损坏 `429` 响应被重放的问题。25 项 Codex 与 5 项中间件回归通过；故障注入的真实 HTTP 上游验证 A `429` → B `200`、有界重试和隔离，生产 Responses、文字续接及 Anthropic 推理均返回 `200`，未人为耗尽生产账号额度。

新增 Codex 账号池的 Anthropic Messages 兼容入口 `/codex/v1/messages`，支持文本、图片、增量 SSE 和工具调用往返；复用原有授权、代理及账号隔离，工具关联的隐藏推理仅保留在服务器端。兼容鉴权错误格式，修复日志中的工具参数拼接及缓存输入计数。`count_tokens` 明确返回 `501`，输出预算等协议差异在接入指南与 API Reference 中说明。

接入指南新增 Codex Anthropic 协议与 Claude Code 配置，使用真实模型目录、正确的 `/codex` Base URL 和仅引用环境变量的网关 Key。验证包括官方 Anthropic SDK 的文本/图片/流式工具往返、Claude Code 2.1.263 的真实 Read 工具执行，以及桌面、390px 手机、深色、中英文及复制操作。Codex 15 项、中间件 5 项回归通过，原生 Responses 仍正常。

修复 Codex 将完整文字历史中的 `assistant` 消息误判为账号绑定续接、导致服务重启后请求返回 `409` 的问题。文字历史可重新建立绑定；未知响应 ID、加密状态、工具回传及失效账号的隔离检查保持不变。回归覆盖重启恢复及未知状态拒绝；生产环境验证流式与非流式推理均返回 `200`。

移除系统设置中的“关于”标签、介绍页面和赞助弹窗，并清理专用状态与版本查询。其余五个设置标签及保存行为保持不变；仓库许可证与文档署名保留。

控制台界面统一：分组侧栏、可折叠桌面导航、手机抽屉、共享页面标题与浅色/深色语义配色。原有十个页面地址保持不变；Google 与 Codex 使用一致的账号池布局，但各自的接口、配额与调度保持独立。Codex 添加账号和原始用量移入对话框，已有账号优先展示。

网关配置按服务与认证、路由与调度、高级设置、模型与集成分组；设置与接入指南采用统一页面结构。网关密钥、管理密码及配置预览默认遮罩，显式显示后自动重新隐藏；用户令牌页移除透明度入场动画。用量图表默认显示前六项及精确汇总的“其他”，可展开全部系列。修复手机表格与提示框越界，以及深色模式下部分额度卡片和表头仍使用浅色背景的问题。

本轮仅修改前端，不涉及数据库或后端协议迁移。已通过生产数据驱动的桌面浏览器、390px 手机视口、深色主题、账号弹窗、配置分组、密钥遮罩、图表展开与接入指南复制验证；未执行账号删除、凭据轮换等破坏性操作。

新增服务端 ChatGPT 订阅 / Codex 通道：独立加密凭据存储、设备码授权、`auth.json` 导入、账号启停及刷新、实际用量与模型目录。Web 后台新增 Codex 页面，支持移动端及中英文。
...
关闭并移除原版自动更新、手动检查与下载安装入口，避免改造版被上游发行包覆盖。导航明确区分“谷歌账号管理”与“Codex账号管理”；删除第三方中转站功能，用内置“API使用说明”替代，提供安全的网关密钥、Google/Gemini/Codex 路由及客户端配置示例。

应用图标更换为 API 节点连接标识，并重新生成 Web、macOS、Windows、Linux、Android 与 iOS 图标资源。

验证覆盖加密与篡改、并发刷新、设备授权取消、SSE 分片及真实 HTTP 收集、会话隔离和账号禁用后的拒绝续接；部署环境已通过真实 Plus 订阅推理、工具回传与压缩调用。
